(function(){
var C=(function(){var el=document.getElementById('content');if(el)return JSON.parse(el.textContent);var no=new URLSearchParams(location.search).get('no');return (window.ARTS||{})[no];})();
...
var NAVL={ja:{prev:'前の記事',next:'次の記事',toc:'目次へ'},ko:{prev:'이전 편',next:'다음 편',toc:'목차로'},en:{prev:'Previous',next:'Next',toc:'Contents'}};
...
function navKeys(){var A=window.ARTS||{};return Object.keys(A).filter(function(k){return A[k]&&A[k].meta&&(A[k].ja||A[k].ko)}).sort(function(a,b){var x=String(a).split('-').map(Number),y=String(b).split('-').map(Number);return (x[0]-y[0])||((x[1]||0)-(y[1]||0))})}
function navTitle(k){var A=window.ARTS||{},a=A[k];if(!a)return '';var l=a[lang]||a.ja||a.ko;return (l&&l.title)||''}
var DEF={ja:{series:'航空旅客運送の実務',voice:'現場のひと言',terms:'今日の用語',tcols:['日本語','한국어','English'],quiz:'確認クイズ',next:'次回：'},ko:{series:'항공 여객운송 실무',voice:'현장 한마디',terms:'오늘의 용어',tcols:['한국어','日本語','English'],quiz:'확인 퀴즈',next:'다음 편: '},en:{series:'Airline Passenger Operations',voice:'Voice from the Field',terms:'Key Terms',tcols:['English','日本語','한국어'],quiz:'Quick Quiz',next:'Next: '}};
var LANGS=['ja','ko','en'];
...
function langBar(){var b=document.getElementById('langBtn');if(!b)return;var g=document.getElementById('langGroup');if(!g){g=document.createElement('span');g.id='langGroup';g.className='langs';g.setAttribute('role','group');g.setAttribute('aria-label','Language');b.parentNode.insertBefore(g,b);b.style.display='none';}
 ...
 g.querySelectorAll('button').forEach(function(x){x.onclick=function(){lang=x.getAttribute('data-l');try{localStorage.setItem('art-lang',lang)}catch(e){}render();window.scrollTo(0,0)}});}
...
function render(){
  ...
  L.sections.forEach(function(s){h+='<section><h2>'+esc(s.h)+'</h2>'+s.blocks.map(function(b){return block(b,L)}).join('')+'</section>'});
  if(L.voice)h+='<section><h2>'+esc(L.voice.h)+'</h2><div class="voice"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 11a7 7 0 0 0 14 0M12 18v4M8 22h8"/></svg><p>'+esc(L.voice.x)+'</p></div></section>';
  ...
  if(L.next){var nx=C.meta.nextno?'view.html?no='+C.meta.nextno:'';h+=(nx?'<a class="next" style="text-decoration:none" href="'+nx+'">':'<div class="next">')+'<span class="pict">'+PLANE+'</span><span>'+esc(L.next)+'</span>'+(nx?'</a>':'</div>');}
  var ks=navKeys(),ix=ks.indexOf(String(C.meta.no)),pv=ix>0?ks[ix-1]:null,nk=(C.meta.nextno&&(window.ARTS||{})[C.meta.nextno])?C.meta.nextno:(ix>=0&&ix<ks.length-1?ks[ix+1]:null),N=NAVL[lang]||NAVL.ja,home=C.meta.home||($('#series')&&$('#series').getAttribute('href'))||'#';
  h+='<nav class="artnav">'+(pv?'<a class="pv" href="view.html?no='+pv+'">← '+esc(N.prev)+'<small>'+esc(pv)+' '+esc(navTitle(pv))+'</small></a>':'<span></span>')+'<a class="toc" href="'+esc(home)+'">☰ '+esc(N.toc)+'</a>'+(nk?'<a class="nx" href="view.html?no='+nk+'">'+esc(N.next)+' →<small>'+esc(nk)+' '+esc(navTitle(nk))+'</small></a>':'<span></span>')+'</nav>';
  $('#main').innerHTML=h;
