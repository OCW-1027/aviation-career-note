...
function draw(){var s=SITE[lang],page=document.body.getAttribute('data-page');
 ...
 if(top)top.innerHTML='<div class="in"><a class="brand" href="index.html">'+logo+'<span>'+esc(s.name)+'</span></a><nav class="menu" aria-label="menu">'+s.menu.map(function(m){return '<a href="'+m[0]+'"'+(m[1]===page?' class="on"':'')+'>'+esc(m[2])+'</a>'}).join('')+'</nav><span class="langs" id="siteLang" role="group" aria-label="Language">'+LANGS.map(function(l){return '<button type="button" data-l="'+l+'" lang="'+l+'" class="'+(l===lang?'on':'')+'" aria-pressed="'+(l===lang)+'">'+LBL[l]+'</button>'}).join('')+'</span></div>';
 var ft=document.getElementById('siteFoot');
 if(ft)ft.innerHTML='<div class="in">'+s.foot.map(function(f){return '<a href="'+f[0]+'">'+esc(f[1])+'</a>'}).join('')+'<span style="margin-left:auto">'+s.copy+'</span></div>';
